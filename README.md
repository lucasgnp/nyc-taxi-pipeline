...
**Existe um tipo de pagamento fora do dicionário fornecido.** O desafio forneceu uma tabela de tipos de pagamento com os códigos de 1 a 6. Mas cerca de 22,5% das corridas têm o código 0, que não está nessa tabela. Ao consultar o dicionário oficial da TLC de 2025, descobri que o código 0 corresponde a "Flex Fare trip", um produto de corrida por aplicativo. Mantive a tabela de referência fiel exatamente ao que o desafio pediu (apenas códigos de 1 a 6) e tratei o código 0 na fato como um valor conhecido mas não classificado, preservando o dado em vez de descartá-lo. Referência: [`Dicionário Oficial da NYC TLC para Yellow Taxi`](https://www.nyc.gov/assets/tlc/downloads/pdfdata_dictionary_trip_records_yellow.pdf).
